...
public:
    ...
    int getCurrentDataSize() { return data.size(); }
    int getRowsPerPage() { return rowsPerPage; }
...
int FormNhapMaXoa(const Font &font, TreeDocGia a, String tieuDe)
{
    ...
    txtBtn.setPosition({btnX + btnW / 2.0f,
                        btnY + btnH / 2.0f - 5.f});
    while (popup.isOpen())
    {
        while (const optional event = popup.pollEvent())
...
bool FormSuaDocGia(const Font &font, TheDocGia &data)
{
    ...
    Text txtBtn(font);
    txtBtn.setString(L"Cập Nhật");
    txtBtn.setPosition({155.f, 358.f});
    txtBtn.setCharacterSize(18);

    Text txtError(font);
    txtError.setFillColor(Color::Red);
    txtError.setCharacterSize(16);
            ...
            if (const auto *mouseBtn = event->getIf<Event::MouseButtonPressed>())
            {
                if (mouseBtn->button == Mouse::Button::Left)
                {
                    ...
                    if (btnSave.getGlobalBounds().contains(mousePos))
                    {
                        ...
                        {
                            txtError.setString(L"Tên Không Hợp Lệ!");
                            continue;
                        }
...
bool FormTraSach(const Font &font, int &outMaThe, string &outMaSach, string &outTenSach)
{
    ...
    btnOk.setPosition({140.f, 300.f});
    btnOk.setFillColor(Color(150, 0, 150));

    Text txtBtn(font);
    txtBtn.setString(L"XÁC NHẬN");
    txtBtn.setCharacterSize(18);
            ...
            if (const auto *mouseBtn = event->getIf<Event::MouseButtonPressed>())
            {
                if (mouseBtn->button == Mouse::Button::Left)
                {
                    ...
                    if (btnOk.getGlobalBounds().contains(mousePos))
                    {
                        ...
                        if (tempMaThe == -1)
                        {
                            txtError.setString(L"Lỗi: Mã Thẻ phải là số!");
                            continue;
                        }
                        if (inputMaSach.content.empty() || inputTenSach.content.empty())
                        {
                            txtError.setString(L"Lỗi: Không được để trống!");
                            continue;
                        }
                        string tempMaSach = "";
...
{
    float winW = 400.f;
    float winH = 450.f; // Chiều cao vừa đủ cho 2 ô
    ...
    txtError.setFillColor(Color::Red);
    txtError.setCharacterSize(16);
    txtError.setPosition({50.f, 300.f});
    txtError.setString("");
...
// Hàm hiển thị popup nhập một chuỗi (Dùng để nhập thông tin liên quan đến sách)
...
// Vừa dùng cho thêm sách hoặc sửa sách (modeEdit = true là sửa)
DauSach *FormNhapSach(const Font &font, DS_DauSach ds_dausach, bool modeEdit, int &slThem, DauSach *dataEdit = NULL)
{
    slThem = 0;
    ...
    btnSave.setFillColor(Color::Blue);
    Text txtSave(font);
    txtSave.setString(L"LƯU");
    txtSave.setPosition(Vector2f(230.f, 540.f));
    txtSave.setCharacterSize(20);
    ...
    while (popup.isOpen())
    {
        while (const optional event = popup.pollEvent())
        {
            ...
            // Xử lý input
            if (!modeEdit)
                inpISBN.handleEvent(*event, popup);
            ...
            if (!modeEdit)
                inpSoLuong.handleEvent(*event, popup);
            ...
            if (const auto *mouseBtn = event->getIf<Event::MouseButtonPressed>())
            {
                if (mouseBtn->button == Mouse::Button::Left)
                {
                    ...
                    if (btnSave.getGlobalBounds().contains(mousePos))
                    {
                        ...
                        {
                            txtErr.setFillColor(Color::Red);
                            txtErr.setString(L"Lỗi: ISBN và tên sách không được để trống!");
                            continue;
                        }
                        // Kiểm tra có phải là ký tự số
                        bool isNumber = true;
                        for (char c : inpISBN.content)
                        {
                            if (!isdigit(c))
                            {
                                isNumber = false;
                        ...
                        {
                            txtErr.setFillColor(Color::Red);
                            txtErr.setString(L"Vui lòng nhập ISBN là chuỗi ký tự số!");
                            continue;
                        }
                        // Kiểm tra đủ độ dài ISBN
                        if (inpISBN.content.length() != 13)
                        {
                            txtErr.setFillColor(Color::Red);
                            txtErr.setString(L"Vui lòng nhập mã ISBN có 13 ký tự số!");
                            continue;
                        }
                        // Kiểm tra trùng ISBN (kiểm tra khi đang thêm đầu sách)
                        if (!modeEdit)
                        {
                            bool biTrung = false;
                            for (int i = 0; i < ds_dausach->n; i++)
                            {
                                if (ds_dausach->nodes[i]->ISBN == inpISBN.content)
                                {
                                    biTrung = true;
                                    break;
                                }
                            ...
                            {
                                txtErr.setFillColor(Color::Red);
                                txtErr.setString(L"Lỗi: ISBN này đã tồn tại trong thư viện!");
                                continue; // Dừng lại, không cho lưu, giữ nguyên Form
                            }
                        }
                        // ------------------------------------
                        ...
                        result->TENSACH = inpTen.content;
                        chuanhoachuoi(result->TENSACH);
                        result->TACGIA = inpTacGia.content;
                        chuanhoachuoi(result->TACGIA);
                        result->THELOAI = inpTheLoai.content;
                        chuanhoachuoi(result->THELOAI);
                        try
                        {
                            result->SOTRANG = stoi(inpTrang.content);
                            result->NAMXUATBAN = stoi(inpNam.content);
                            // Validate số âm
                            if (result->SOTRANG <= 0 || result->NAMXUATBAN <= 0)
                                throw 1;

                            if (!modeEdit)
                            {
                                result->slsach = stoi(inpSoLuong.content);
                                if (result->slsach <= 0)
                                    throw 1;
                            }
                            else
                                result->slsach = slHientai + slThem;
                        }
                        catch (...)
                        {
                            txtErr.setFillColor(Color::Red);
                            txtErr.setString(L"Lỗi: Số trang, Năm, Số lượng phải là số dương");
                            delete result;
                            continue;
                        }
        ...
        inpTrang.draw(popup);
        inpNam.draw(popup);
        inpTacGia.draw(popup);
        inpTheLoai.draw(popup);
        inpSoLuong.draw(popup);
        if (modeEdit)
        {
...
// Thêm đầu sách vào mảng và tự tạo các bản sao sách con
void ThemDauSach(const Font &font, DS_DauSach &ds_dausach, DauSach *p)
{
    ...
    for (int i = ds_dausach->n; i > pos; i--)
    {
        ds_dausach->nodes[i] = ds_dausach->nodes[i - 1];
    }
    ds_dausach->nodes[pos] = p;
    ds_dausach->n++;
}
...
void FormXoaDauSach(const Font &font, DS_DauSach &ds_dausach, string isbn_can_xoa)
{
    ...
    {
        // Thực hiện xóa bộ nhớ danh sách liên kết con (trạng thái sách con 0 hoặc 2)
...
void FormDieuChinhSach(const Font &font, DS_DauSach &ds_dausach, string isbn)
{
    for (int i = 0; i < ds_dausach->n; i++)
    {
        if (ds_dausach->nodes[i]->ISBN != isbn)
        {
            ShowMessage(font, L"Lỗi: Không tìm thấy ISBN này!");
            return;
        }
    ...
    if (oldData != NULL)
    {
        ...
        DauSach *newData = FormNhapSach(font, ds_dausach, true, slThem, oldData);
        ...
        if (newData != NULL)
        {
            ...
            if (slThem > 0)
            {
                ThemBanSao(ds_dausach, oldData->ISBN, slThem);
            }
            ...
            if (tenThayDoi)
            {
                // Lưu con trỏ sách hiện tại
                DauSach *pCurrent = ds_dausach->nodes[index];

                // Xóa sách khỏi vị trí cũ (Dồn mảng sang trái)
                for (int i = index; i < ds_dausach->n - 1; i++)
                {
                    ds_dausach->nodes[i] = ds_dausach->nodes[i + 1];
                }
                ds_dausach->n--; // Giảm số lượng tạm thời

                // Tìm vị trí mới thích hợp (Insertion Sort theo tên sách)
                int newPos = ds_dausach->n;
                for (int i = 0; i < ds_dausach->n; i++)
                {
                    if (pCurrent->TENSACH < ds_dausach->nodes[i]->TENSACH)
                    {
                        newPos = i;
                        break;
                    }
                ...
                // Dồn mảng sang phải để tạo chỗ trống tại newPos
                for (int i = ds_dausach->n; i > newPos; i--)
                {
                    ds_dausach->nodes[i] = ds_dausach->nodes[i - 1];
                }

                // Chèn sách vào vị trí mới
                ds_dausach->nodes[newPos] = pCurrent;
                ds_dausach->n++; // Tăng lại số lượng
            }
        }
    }
}
    ...
    while (popup.isOpen())
    {
        ...
        while (const optional event = popup.pollEvent())
        {
            ...
            if (const auto *key = event->getIf<Event::KeyPressed>())
            {
                if (key->code == Keyboard::Key::Escape)
                    popup.close();
                if (key->code == Keyboard::Key::PageUp)
                    table.prevPage();
                if (key->code == Keyboard::Key::PageDown)
                    table.nextPage();
            }
...
// Hàm hiển thị popup chọn kiểu sắp xếp
int FormChonSapXep(const Font &font)
{
    ...
    // Tạo 2 nút lựa chọn
    Button btnTheoTen(50.f, 100.f, 140.f, 45.f, "TÊN", font, Color(70, 70, 80));
    Button btnTheoTheLoai(210.f, 100.f, 140.f, 45.f, "THỂ LOẠI", font, Color(70, 70, 80));
    ...
    while (popup.isOpen())
    {
        ...
        btnTheoTen.update(mousePos);
        btnTheoTheLoai.update(mousePos);
        ...
        while (const optional event = popup.pollEvent())
        {
            ...
            if (const auto *mouseBtn = event->getIf<Event::MouseButtonPressed>())
            {
                if (mouseBtn->button == Mouse::Button::Left)
                {
                    if (btnTheoTen.isClicked(mousePos))
                    {
                        popup.close();
                        return 1;
                    }
                    if (btnTheoTheLoai.isClicked(mousePos))
                    {
                        popup.close();
                        return 2;
                    }
        ...
        popup.draw(title);
        btnTheoTen.draw(popup);
        btnTheoTheLoai.draw(popup);
        popup.display();
    }
    return 0;
}

void FormThanhLySach(const Font &font, DS_DauSach &ds_dausach)
{
    // Nhập ISBN
    string isbn = FormNhapChuoi(font, "NHẬP ISBN CẦN THANH LÝ:", "ISBN:");
    if (isbn.empty())
        return;
    ...
    // Tạo cửa sổ popup
    RenderWindow popup(VideoMode({1000, 700}), L"Thanh Lý Sách Bản Sao", Style::Titlebar | Style::Close);
    popup.setFramerateLimit(60);

    // Tiêu đề
    Text txtTitle(font);
    txtTitle.setString(L"DANH SÁCH BẢN SAO (THANH LÝ)");
    txtTitle.setCharacterSize(26);
    txtTitle.setFillColor(Color::Yellow);
    txtTitle.setStyle(Text::Bold);
    FloatRect tr = txtTitle.getLocalBounds();
    txtTitle.setPosition({(1000.f - tr.size.x) / 2.0f, 20.f});

    // Thông tin sách
    Text txtInfo(font);
    txtInfo.setCharacterSize(18);
    txtInfo.setFillColor(Color::White);
    txtInfo.setPosition({50.f, 70.f});

    // --- CẤU HÌNH BẢNG ---
    float tableY = 150.f;
    TableDisplay table(50.f, tableY, 10, "C:/Windows/Fonts/segoeui.ttf");

    // Header bảng
    vector<string> headers = {"STT", "XỬ LÝ", "MÃ SÁCH", "TRẠNG THÁI", "VỊ TRÍ"};
    vector<int> widths = {6, 8, 26, 20, 18};
    table.setHeaders(headers, widths);
    ...
    float btnW = 130.f;
    float btnH = 40.f;
    Button btnPrev(360.f, btnY, btnW, btnH, "<< TRƯỚC", font, Color(100, 100, 100));
    Button btnNext(510.f, btnY, btnW, btnH, "SAU >>", font, Color(100, 100, 100));
    ...
    auto RefreshTable = [&]()
    {
        ...
        string infoText = "Tên Sách: " + d->TENSACH +
                          "\nSố lượng: " + to_string(d->slsach) +
                          "  |  Đã mượn: " + to_string(d->slm);
        txtInfo.setString(sf::String::fromUtf8(infoText.begin(), infoText.end()));
    };
    ...
    while (popup.isOpen())
    {
        ...
        while (const optional event = popup.pollEvent())
        {
            ...
            if (const auto *mouseBtn = event->getIf<Event::MouseButtonPressed>())
            {
                if (mouseBtn->button == Mouse::Button::Left)
                {
                    ...
                    // --- XỬ LÝ CLICK NÚT THANH LÝ ---
                    float col1Width = widths[0] * 12.0f;
                    float col2Width = widths[1] * 12.0f;
                    float btnWidth = 24.f;
                    float btnX = 50.f + col1Width + (col2Width - btnWidth) / 2.0f;
                    float startDrawY = tableY + 40.f;
                    float lineHeight = 40.f;

                    int rowsOnPage = 10;
                    int currentPage = table.getCurrentPage();
                    int totalData = table.getCurrentDataSize();
                    int currentRowsCount = min(rowsOnPage, totalData - (currentPage * rowsOnPage));

                    for (int i = 0; i < currentRowsCount; i++)
                    {
                        FloatRect btnRect(Vector2f(btnX, startDrawY + i * lineHeight + 8.f), Vector2f(btnWidth, btnWidth));

                        if (btnRect.contains(mousePos))
                        {
                            // Tìm sách tương ứng
                            int realIndex = currentPage * rowsOnPage + i;
                            SACH pScan = d->dms;
                            int count = 0;
                            string maSachChon = "";
                            ...
                            while (pScan != NULL)
                            {
                                if (count == realIndex)
                                {
                                    maSachChon = pScan->data.MASACH;
                                    trangThaiHienTai = pScan->data.trangthai;
                                    break;
                                }
                                count++;
                                pScan = pScan->next;
                            ...
                            if (maSachChon != "")
                            {
                                ...
                                {
                                    ShowMessage(font, L"Sách này đã được thanh lý rồi!");
                                }
                                else if (ShowConfirm(font, L"Bạn có muốn thanh lý sách: \n" + sf::String::fromUtf8(maSachChon.begin(), maSachChon.end())))
                                {
                                    // GỌI HÀM MỚI Ở ĐÂY
                                    int kq = ThanhLySach(d, maSachChon);

                                    if (kq == 1)
                                    {
                                        ShowMessage(font, L"Đã thanh lý sách thành công!");
                                        RefreshTable(); // Load lại bảng để thấy chữ "ĐÃ THANH LÝ"
                                    }
                                    else if (kq == -1)
                                    {
                                        ShowMessage(font, L"Lỗi: Sách đang được mượn, không thể thanh lý!");
                                    }
                                }
                            }
                        }
        ...
        for (int k = 0; k < currentRowsCount; k++)
        {
            // Lấy trạng thái thực tế để vẽ màu nút khác nhau (Optional)
            // Ở đây vẽ màu Cam để biểu thị "Thay đổi trạng thái" thay vì màu Đỏ "Xóa"
            RectangleShape rectDel(Vector2f(btnWidth, btnWidth));
            rectDel.setPosition(Vector2f(btnX, startDrawY + k * lineHeight + 8.f));

            if (rectDel.getGlobalBounds().contains(mousePos))
                rectDel.setFillColor(Color(255, 140, 0)); // Cam sáng
            else
                rectDel.setFillColor(Color(200, 100, 0)); // Cam tối

            // Vẽ dấu gạch ngang (Trừ)
            RectangleShape minus(Vector2f(12.f, 3.f));
            minus.setOrigin(Vector2f(6.f, 1.5f));
            minus.setPosition(Vector2f(btnX + btnWidth / 2.0f, startDrawY + k * lineHeight + 8.f + btnWidth / 2.0f));
            minus.setFillColor(Color::White);
...
void loadDauSachToTable(DS_DauSach &ds_dausach, TableDisplay &table)
{
    table.clear();
    ...
    for (int i = 0; i < ds_dausach->n; i++)
    {
        ...
        row.push_back(ds_dausach->nodes[i]->ISBN);
        row.push_back(ds_dausach->nodes[i]->TENSACH);
        row.push_back(to_string(ds_dausach->nodes[i]->SOTRANG));
        row.push_back(ds_dausach->nodes[i]->TACGIA);
        row.push_back(to_string(ds_dausach->nodes[i]->NAMXUATBAN));
        row.push_back(ds_dausach->nodes[i]->THELOAI);
        row.push_back(to_string(ds_dausach->nodes[i]->slsach));
        table.addRow(row);
    }
}
...
void loadMuonTraToTable(MT head, TableDisplay &table, string ten, int x)
{
    ...
    while (p != NULL)
    {
        ...
        if (p->mt.trangthai2 == 0)
        {
            ...
            else if (p->mt.trangthai2 == 2)
                row.push_back("ĐÃ MẤT");
            table.addRow(row);
            coSach = true;
        }
        p = p->next;
...
void inquahan(DS_TheDocgia &a, TableDisplay &b)
{ // câu i
...
void top10book(DS_DauSach &a, TableDisplay &b)
{ // 10 sách dc mượn nhiều nhất (j)
...
int main()
{
    ...
    vector<int> widthSach = {5, 15, 40, 10, 25, 14, 25, 14};
    ...
    int currentSortMode = 1; //(1: Theo tên, 2: Theo thể loại)
    ...
    while (Window.isOpen())
    {
        ...
        btnTabSach.update(mousePos);
        btnTabDocGia.update(mousePos);
        btnTabMuon.update(mousePos);
        btnPrevPage.update(mousePos);
        btnNextPage.update(mousePos);
        btnSaveFile.update(mousePos);

        if (currentTab == TAB_DOCGIA)
        {
            btnAdd.update(mousePos);
            btnDel.update(mousePos);
            btnEdit.update(mousePos);
            btnlock.update(mousePos);
            btnintheoten.update(mousePos);
            btnintheomathe.update(mousePos);
        }
        else if (currentTab == TAB_DAUSACH)
        {
            btnSach_Them.update(mousePos);
            btnSach_Xoa.update(mousePos);
            btnSach_DieuChinh.update(mousePos);
            btnSach_ThongTinSach.update(mousePos);
            btnSach_SapXep.update(mousePos);
            btnSach_ThanhLySach.update(mousePos);
        }
        else if (currentTab == TAB_MUONTRA)
        {
            btnMuon_in.update(mousePos);
            btnMuon_Tra.update(mousePos);
            btnMuon_Muon.update(mousePos);
            btnMuon_QUAHAN.update(mousePos);
            btnMuon_Top10Sach.update(mousePos);
        }
        ...
        while (const optional event = Window.pollEvent())
        {
            ...
            {
                if (key->code == Keyboard::Key::PageUp)
                    tableDisplay.prevPage();
                if (key->code == Keyboard::Key::PageDown)
                    tableDisplay.nextPage();
            }
            ...
            if (const auto *mouseBtn = event->getIf<Event::MouseButtonPressed>())
            {
                if (mouseBtn->button == Mouse::Button::Left)
                {
                    ...
                    if (btnTabDocGia.isClicked(clickPos))
                    {
                        currentTab = TAB_DOCGIA;
                        RefreshList();
                    }
                    if (btnTabSach.isClicked(clickPos))
                    {
                        currentTab = TAB_DAUSACH;
                        RefreshList();
                    }
                    if (btnTabMuon.isClicked(clickPos))
                    {
                        currentTab = TAB_MUONTRA;
                        currentViewMaThe = -1;
                        RefreshList();
                    }

                    if (btnPrevPage.isClicked(clickPos))
                        tableDisplay.prevPage();
                    if (btnNextPage.isClicked(clickPos))
                        tableDisplay.nextPage();
                    ...
                    else if (currentTab == TAB_DAUSACH)
                    {
                        ...
                        {
                            int slThem = 0;
                            DauSach *pMoi = FormNhapSach(font, dsdausach, false, slThem);
                            if (pMoi != NULL)
                            {
                        ...
                        {
                            FormThanhLySach(font, dsdausach);
                            RefreshList();
                        }
                    }
                    else if (currentTab == TAB_MUONTRA)
        ...
        btnTabSach.draw(Window);
        btnTabDocGia.draw(Window);
        btnTabMuon.draw(Window);
        ...
        if (currentTab == TAB_DOCGIA)
        {
            ...
            btnEdit.draw(Window);
            btnlock.draw(Window);
            btnintheoten.draw(Window);
            btnintheomathe.draw(Window);
        }
        else if (currentTab == TAB_DAUSACH)
        {
            btnSach_Them.draw(Window);
            btnSach_Xoa.draw(Window);
            btnSach_DieuChinh.draw(Window);
            btnSach_ThongTinSach.draw(Window);
            btnSach_SapXep.draw(Window);
            btnSach_ThanhLySach.draw(Window);
        }
        else if (currentTab == TAB_MUONTRA)
        {
            btnMuon_in.draw(Window);
            btnMuon_Tra.draw(Window);
            btnMuon_Muon.draw(Window);
            btnMuon_QUAHAN.draw(Window);
            btnMuon_Top10Sach.draw(Window);
